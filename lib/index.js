 ...
 * 4) HTTP 路由 POST /warp/debug：客户端关键状态上报（写入 debug.jsonl，排查显示问题用）。
 */
...
// ---- 调试上报（临时诊断通道，排查「新会话首条命令不显示」用）----
const DEBUG_FILE = '/Users/beck.lee/Desktop/dsh-workspace/dsh-warp-input/debug.jsonl'
function debugRecord(payload) {
  try {
    appendFileSync(DEBUG_FILE, JSON.stringify({ t: Date.now(), pid: process.pid, ...payload }) + '\n')
  } catch (e) {}
}
...
export function apply(ctx) {
  ...
  // ---- /warp/debug：客户端状态上报（写入 debug.jsonl）----
  ...
  ctx.effect(() => webServer.register({ kind: 'exact', path: '/warp/debug', handler: debugHandler }))
