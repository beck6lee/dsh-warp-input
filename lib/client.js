...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 会话是否已有真实对话内容（user/assistant/command 节点）。
    // 空白会话可能只有 context/steering 节点——它们不算内容，不应阻止恢复。
    var hasConversationContent = function (nodes) {
      if (!nodes || nodes.length === 0) return false
      for (var i = 0; i < nodes.length; i++) {
        var n = nodes[i]
        if (n && (n.kind === 'user' || n.kind === 'assistant' || n.kind === 'command')) return true
      }
      return false
    }
    ...
    // 调试上报：POST /warp/debug（host 写入 debug.jsonl，排查显示问题）
    ...
    function apply(ctx) {
      ...
      if (!errorHooked) {
        ...
        try {
          window.addEventListener('error', function (e) {
            debugLog({ ev: 'window-error', msg: String((e && e.message) || e), stack: String((e && e.error && e.error.stack) || '').slice(0, 600) })
          })
          window.addEventListener('unhandledrejection', function (e) {
            debugLog({ ev: 'unhandled-rejection', msg: String((e && e.reason && (e.reason.message || e.reason)) || e) })
          })
          var origError = console.error
          console.error = function () {
            try {
              var args = Array.prototype.slice.call(arguments)
              var msg = args.map(function (a) { return typeof a === 'string' ? a : (a && a.message) || String(a) }).join(' ')
              if (msg.length > 900) msg = msg.slice(0, 900)
              debugLog({ ev: 'console-error', msg: msg })
            } catch (e2) {}
      ...
      function WarpComposer(props) {
        ...
        var nodes = typeof useSession === 'function'
          ...
          : []
        // 会话窗口状态（调试用）
        var openState = typeof useSession === 'function'
          ? useSession(function (s) { return s && s.openState })
          : undefined
        var nodeKinds = nodes.map(function (n) { return n && n.kind })
        // 调试用：渲染层证据（order 是否含命令节点、命令节点可见性/位置、DOM 实际行）
        var renderInfo = typeof useSession === 'function'
          ? useSession(function (s) {
              var c = s && s.chat
              if (!c) return { orderKinds: [], cmdVis: [] }
              var kinds = []
              var keys = c.order || []
              for (var i = 0; i < keys.length && i < 20; i++) {
                var n = c.nodes && c.nodes.get ? c.nodes.get(keys[i]) : undefined
                kinds.push(n ? n.kind : '?')
              }
              var cmdVis = []
              var all = c.nodes && c.nodes.values ? c.nodes.values() : []
              for (var j = 0; j < all.length; j++) {
                var nn = all[j]
                if (nn && nn.kind === 'command') {
                  cmdVis.push({ vis: nn.visibility, loc: nn.location && nn.location.kind, seq: nn.anchorSeq })
                }
              }
              return { orderKinds: kinds, cmdVis: cmdVis }
            })
          : { orderKinds: [], cmdVis: [] }
        var domRows = (function () {
          try {
            var els = document.querySelectorAll('[data-chat-flow-kind]')
            var out = []
            for (var i = 0; i < els.length && i < 15; i++) out.push(els[i].getAttribute('data-chat-flow-kind'))
            return out
          } catch (e) { return [] }
        })()
        var domRowCount = (function () {
          try { return document.querySelectorAll('[data-chat-flow-key]').length } catch (e) { return -1 }
        })()
        // 调试用：视图 tab 状态 / 聊天列是否存在 / 会话区域文本
        var tabsInfo = (function () {
          try {
            var out = []
            var els = document.querySelectorAll('[role="tab"]')
            for (var i = 0; i < els.length && i < 6; i++) {
              out.push((els[i].getAttribute('aria-selected') === 'true' ? '>' : ' ') + (els[i].textContent || '').trim())
            }
            return out
          } catch (e) { return [] }
        })()
        var chatFlowCount = (function () {
          try { return document.querySelectorAll('[data-chat-flow]').length } catch (e) { return -1 }
        })()
        var convText = (function () {
          try {
            var el = document.querySelector('[data-conversation-scroll]')
            return el ? (el.innerText || '').replace(/\s+/g, ' ').slice(0, 150) : '(no scroll el)'
          } catch (e) { return '(err)' }
        })()
        ...
        // 新会话首条命令的显示恢复
        var recoverState = React.useState(false)
        var pendingRecovery = recoverState[0]
        var setPendingRecovery = recoverState[1]
        // 恢复目标（提交时捕获，避免恢复执行时用户已切换会话）与单次执行守卫
        var recoveryRanRef = React.useRef(false)
        var recoveryTargetRef = React.useRef(null)
        // 最新状态快照（供延时回调读取当前值，避免闭包过期）
        var stateRef = React.useRef({})
        stateRef.current = {
          sessionId: sessionId,
          openState: openState,
          nodeKinds: nodeKinds,
          orderKinds: renderInfo.orderKinds,
          cmdVis: renderInfo.cmdVis,
          domRows: domRows,
          domRowCount: domRowCount,
          tabsInfo: tabsInfo,
          chatFlowCount: chatFlowCount,
          convText: convText,
        }
        ...
        // 调试上报：挂载时快照（每次会话切换）
        React.useEffect(function () {
          debugLog({ ev: 'mount', sessionId: sessionId, openState: openState, nodeKinds: nodeKinds, orderKinds: renderInfo.orderKinds, cmdVis: renderInfo.cmdVis, domRows: domRows, domRowCount: domRowCount, tabsInfo: tabsInfo, chatFlowCount: chatFlowCount, convText: convText })
        }, [sessionId])
        ...
        React.useEffect(function () {
          ...
          var sessionsCtx = ctx.sessions
          if (!sessionsCtx || typeof sessionsCtx.open !== 'function') return
          if (hasConversationContent(nodes)) return // 当前会话有真实内容，不打扰
        ...
        // 恢复：直接强制目标会话重拉历史（等价于 repairGap 的尾部重拉，但不依赖 seq 缺口）。
        // 被丢弃的命令事件已在 host 持久化，重拉后随历史窗口显示。
        var recoverBySessionSwitch = function () {
          var sessionsCtx = ctx.sessions
          var target = recoveryTargetRef.current || sessionId
          var session = null
          try {
            var manager = sessionsCtx && sessionsCtx.manager
            if (manager && typeof manager.get === 'function') session = manager.get(target)
          } catch (e) {}
          if (session && typeof session.resync === 'function' && typeof session.open === 'function') {
            console.log('[warp] 恢复触发：强制重拉会话历史', { target: target, openState: session.openState })
            debugLog({ ev: 'recover-action', target: target, openState: session.openState, action: session.openState === 'open' ? 'resync' : 'open' })
            try {
              if (session.openState === 'open') session.resync()
              else session.open()
              return
            } catch (e) {}
          }
          // 兜底：重载页面（重开后从历史恢复命令结果）
          debugLog({ ev: 'recover-fallback-reload', target: target })
        ...
        var submitCommand = function (draft) {
          ...
          // 新会话（无对话内容）首条命令：命令事件可能被客户端丢弃（会话窗口未就绪），
          // 启用显示恢复（节点未在 1.5s 内出现则强制重拉历史）
          var willRecover = !hasConversationContent(nodes)
          debugLog({ ev: 'submit', sessionId: sessionId, cmd: cmd, openState: openState, nodeKinds: nodeKinds, orderKinds: renderInfo.orderKinds, cmdVis: renderInfo.cmdVis, domRows: domRows, domRowCount: domRowCount, tabsInfo: tabsInfo, chatFlowCount: chatFlowCount, convText: convText, willRecover: willRecover })
          // 提交 3s 后的快照（观察新命令节点是否进入 order / DOM）
          setTimeout(function () {
            var st3 = stateRef.current
            debugLog({ ev: 'post-submit', sessionId: st3.sessionId, openState: st3.openState, nodeKinds: st3.nodeKinds, orderKinds: st3.orderKinds, cmdVis: st3.cmdVis, domRows: st3.domRows, domRowCount: st3.domRowCount, tabsInfo: st3.tabsInfo, chatFlowCount: st3.chatFlowCount, convText: st3.convText })
          }, 3000)
          if (willRecover) {
            console.log('[warp] 命令提交于空白会话，启用显示恢复', { sessionId: sessionId })
            recoveryRanRef.current = false
            recoveryTargetRef.current = sessionId
            setPendingRecovery(true)
          }
        }

        // 显示恢复：节点出现即完成；1.5s 内未出现则强制重拉历史（每次提交最多一次）
        React.useEffect(function () {
          if (!pendingRecovery) return
          if (hasConversationContent(nodes)) {
          ...
          var timer = setTimeout(function () {
            ...
            debugLog({ ev: 'recover', sessionId: st.sessionId, openState: st.openState, nodeKinds: st.nodeKinds })
            recoverBySessionSwitch()
            // 恢复后 2.5s 上报结果
            setTimeout(function () {
              var st2 = stateRef.current
              debugLog({ ev: 'post-recover', sessionId: st2.sessionId, openState: st2.openState, nodeKinds: st2.nodeKinds })
            }, 2500)
          }, 1500)
          return function () { clearTimeout(timer) }
        }, [nodes, pendingRecovery])
        ...
        return React.createElement('div', { className: 'warp-composer' },
          ...
          React.createElement('div', { className: 'warp-row' },
            ...
            (isCommand && historyList.length > 0)
              ...
              : null,
            pendingRecovery
              ? React.createElement('span', { className: 'warp-hint' }, '正在恢复命令显示…')
              : null,
