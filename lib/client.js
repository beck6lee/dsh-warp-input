  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function WarpComposer(props) {
        ...
        // 恢复：切到其他会话再切回，强制当前会话重拉历史（命令事件被丢弃时恢复显示）
        var recoverBySessionSwitch = function () {
          var sessionsCtx = ctx.get('sessions')
          if (!sessionsCtx || typeof sessionsCtx.search !== 'function' || typeof sessionsCtx.open !== 'function') return
          console.log('[warp] 恢复触发：1.5s 内命令节点未出现，切换会话强制重拉历史')
          try {
            var controller = new AbortController()
            sessionsCtx.search('', controller.signal).then(function (res) {
              var items = res && res.value && res.value.items ? res.value.items : []
              var other = null
              for (var i = 0; i < items.length; i++) {
                if (items[i].sessionId && items[i].sessionId !== sessionId) { other = items[i].sessionId; break }
              }
              if (other) {
                sessionsCtx.open(other)
                setTimeout(function () { sessionsCtx.open(sessionId) }, 250)
              }
            }).catch(function () {})
          } catch (e) {}
        }
        ...
        var submitCommand = function (draft) {
          ...
          // 新会话（无对话节点）首条命令：命令事件可能被客户端丢弃（会话窗口未就绪），
          // 启用显示恢复（节点未在 1.5s 内出现则切换会话强制重拉历史）
          if (nodes.length === 0) {
        ...
        // 显示恢复：节点出现即完成；1.5s 内未出现则切换会话重拉历史
        React.useEffect(function () {
