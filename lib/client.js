  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      var css = [
        ...
        // 命令结果以「用户气泡 + 普通消息」呈现，与正常对话一致
        '.warp-msg { display:flex; flex-direction:column; gap:6px; min-width:0; }',
        '.warp-msg-user { display:flex; flex-direction:column; align-items:flex-end; gap:6px; }',
        '.warp-msg-stack { display:flex; flex-direction:column; align-items:flex-end; gap:8px; min-width:0; max-width:min(525px,82%); }',
        '.warp-msg-bubble { background:var(--dsw-specific-bubble); max-width:100%; color:var(--dsw-alias-label-primary); border-radius:22px; padding:10px 16px; font-size:16px; line-height:24px; white-space:pre-wrap; word-break:break-word; }',
        '.warp-msg-answer { color:var(--dsw-alias-label-primary); font-size:16px; line-height:28px; min-width:0; }',
        '.warp-msg-out { margin:0; font-family:inherit; white-space:pre-wrap; word-break:break-word; max-height:480px; overflow:auto; }',
        '.warp-msg-out[data-error] { color:var(--dsw-alias-state-error-primary); }',
        '.warp-msg-run { color:var(--dsw-alias-label-tertiary); font-size:13px; line-height:20px; }',
      ].join('\n')
      ...
      // ---- /run 命令的对话流视图：以「用户气泡 + 普通消息」呈现，与正常对话一致 ----
      function RunCommandView(props) {
        var node = props.node
        var args = (node && node.args ? String(node.args).trim() : '')
        var outcome = node ? node.outcome : null
        // 去掉 host 附加的 [exit N]/[TIMEOUT] 标记，输出更接近自然对话
        var text = (outcome && outcome.text ? String(outcome.text) : '').replace(/\s*\[(exit \d+|TIMEOUT)\]\s*$/, '')
        var cmdText = args || 'run'
        var userBubble = React.createElement('div', { className: 'warp-msg-user' },
          React.createElement('div', { className: 'warp-msg-stack' },
            React.createElement('div', { className: 'warp-msg-bubble' }, cmdText),
          ),
        )
        if (outcome === null) {
          return React.createElement('div', { className: 'warp-msg' },
            userBubble,
            React.createElement('div', { className: 'warp-msg-run' }, '执行中…'),
          )
        }
        return React.createElement('div', { className: 'warp-msg' },
          userBubble,
          React.createElement('div', { className: 'warp-msg-answer' },
            React.createElement('pre', {
              className: 'warp-msg-out',
              'data-error': outcome.kind === 'error' ? true : undefined,
            }, text),
          ),
        )
      }
