  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function WarpComposer(props) {
        ...
        // 幽灵补全：命令模式、单行、非 CJK、末尾非空格时，防抖请求 /warp/complete
        React.useEffect(function () {
          var t = draft.trim()
          var eligible = isCommand && t && !t.endsWith(' ') && t.indexOf('\n') === -1 && !hasCJK(t)
          if (!eligible) {
            setGhost(null)
            return
          }
        ...
        var showGhost = isCommand && ghost && draft.trim() && draft.indexOf('\n') === -1 && (draft + ghost).length < 80
        ...
        var onKeyDown = function (e) {
          ...
          if (e.key === 'Tab' && isCommand && ghost && !e.nativeEvent.isComposing) {
            e.preventDefault()
