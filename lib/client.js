    ...
    function apply(ctx) {
      ...
      function WarpComposer(props) {
        ...
        var submitCommand = function (draft) {
          ...
          // 空白会话解锁：命令走斜杠直通路径，不产生 user/message 事件。DSH 客户端
          // 把「仅含命令节点」的会话判为 blank（hasVisibleConversationContent 排除
          // command 节点），ConversationSession 在 blank && composerPhase==="blank"
          // 时直接隐藏整个对话视图（命令节点其实一直在）。
          // 置 promptAttempted=true 使 composerPhase 变为 engaging，解锁视图显示命令节点。
          try {
            var sctx = ctx.sessions
            var mgr = sctx && sctx.manager
            var sess = mgr && typeof mgr.get === 'function' ? mgr.get(sessionId) : undefined
            if (sess && !sess.promptAttempted) {
              sess.promptAttempted = true
              if (sess.notifier && typeof sess.notifier.markDirty === 'function') sess.notifier.markDirty()
              debugLog({ ev: 'unblank', sessionId: sessionId })
            }
          } catch (e) {}
          debugLog({ ev: 'submit', sessionId: sessionId, cmd: cmd, openState: openState, nodeKinds: nodeKinds, orderKinds: renderInfo.orderKinds, cmdVis: renderInfo.cmdVis, domRows: domRows, domRowCount: domRowCount, tabsInfo: tabsInfo, chatFlowCount: chatFlowCount, convText: convText })
          // 提交 3s 后的快照（验证命令节点渲染）
          setTimeout(function () {
