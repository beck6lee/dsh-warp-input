  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function WarpComposer(props) {
        ...
        // 调试上报：挂载时快照（每次会话切换）
        React.useEffect(function () {
          debugLog({ ev: 'mount', sessionId: sessionId, openState: openState, nodeKinds: nodeKinds, orderKinds: renderInfo.orderKinds, cmdVis: renderInfo.cmdVis, domRows: domRows, domRowCount: domRowCount })
        }, [sessionId])
        ...
        var submitCommand = function (draft) {
          ...
          debugLog({ ev: 'submit', sessionId: sessionId, cmd: cmd, openState: openState, nodeKinds: nodeKinds, orderKinds: renderInfo.orderKinds, cmdVis: renderInfo.cmdVis, domRows: domRows, domRowCount: domRowCount, willRecover: willRecover })
          // 提交 3s 后的快照（观察新命令节点是否进入 order / DOM）
          setTimeout(function () {
            var st3 = stateRef.current
            debugLog({ ev: 'post-submit', sessionId: st3.sessionId, openState: st3.openState, nodeKinds: st3.nodeKinds, orderKinds: st3.orderKinds, cmdVis: st3.cmdVis, domRows: st3.domRows, domRowCount: st3.domRowCount })
          }, 3000)
          if (willRecover) {
