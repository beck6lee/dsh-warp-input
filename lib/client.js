  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function WarpComposer(props) {
        ...
        stateRef.current = { sessionId: sessionId, openState: openState, nodeKinds: nodeKinds }
        ...
        // 调试上报：挂载时快照（每次会话切换）
        React.useEffect(function () {
          debugLog({ ev: 'mount', sessionId: sessionId, openState: openState, nodeKinds: nodeKinds })
        }, [sessionId])
        ...
        var submitCommand = function (draft) {
          ...
          debugLog({ ev: 'submit', sessionId: sessionId, cmd: cmd, openState: openState, nodeKinds: nodeKinds, willRecover: willRecover })
          if (willRecover) {
