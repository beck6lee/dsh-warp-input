...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      var css = [
        '.warp-composer { display:flex; flex-direction:column; gap:8px; width:100%; max-width:var(--dsh-composer-card-max-width, 780px); margin:0 auto; padding:10px 12px 14px; border:1px solid var(--dsw-alias-border-l2); border-radius:16px; background:var(--dsw-alias-bg-base); }',
      ...
      function WarpComposer(props) {
        ...
        var renderStatsLine = function () {
          ...
          if (sessionStats && (sessionStats.turns > 0 || sessionStats.steps > 0)) {
            parts.push(sessionStats.turns + ' 轮 · ' + sessionStats.steps + ' 步')
          }
          if (usage) {
            var input = (usage.uncachedInputTokens || 0) + (usage.cacheReadTokens || 0) + (usage.cacheWriteTokens || 0)
            var out = usage.outputTokens || 0
            if (input > 0 || out > 0) {
              var hit = input > 0 && usage.cacheReadTokens > 0 ? Math.round(usage.cacheReadTokens / input * 100) : null
              var tok = '↑' + formatTokens(input) + ' ↓' + formatTokens(out) + ' tok'
              parts.push(hit !== null ? tok + ' · 缓存 ' + hit + '%' : tok)
            }
          }
          if (pressure && pressure.contextWindow > 0) {
            ...
            if (used !== undefined) {
              var pct = Math.min(100, Math.round(used / pressure.contextWindow * 100))
              parts.push('上下文 ' + pct + '%')
            }
          }
          if (parts.length === 0) return null
          return React.createElement('div', { className: 'warp-stats', title: parts.join(' | ') }, parts.join(' | '))
        }
